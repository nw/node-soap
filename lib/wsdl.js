...
function Element(nsName, attrs) {
  var parts = util.splitNSName(nsName);
...
Element.prototype.startElement = function(stack, nsName, attrs) {
  ...
  var ChildClass = this.allowedChildren[util.splitNSName(nsName).name],
    element = null;
...
Element.prototype.endElement = function(stack, nsName) {
  if (this.nsName === nsName) {
    ...
    if (this !== stack[0]) {
      util.extend(stack[0].xmlns, this.xmlns);
      // delete this.xmlns;
...
MessageElement.prototype.postProcess = function(definitions) {
  ...
  if (part.$element) {
    delete this.parts;
    nsName = util.splitNSName(part.$element);
    ns = nsName.namespace;
    ...
    for (i = 0; part = this.children[i]; i++) {
      ...
      nsName = util.splitNSName(part.$type);
...
OperationElement.prototype.postProcess = function(definitions, tag) {
  ...
  for (var i = 0, child; child = children[i]; i++) {
    ...
    var messageName = util.splitNSName(child.$message).name;
...
BindingElement.prototype.postProcess = function(definitions) {
  var type = util.splitNSName(this.$type).name,
...
ServiceElement.prototype.postProcess = function(definitions) {
  ...
  for (var i = 0, child; child = children[i]; i++) {
    if (child.name !== 'port')
      continue;
    var bindingName = util.splitNSName(child.$binding).name;
...
ElementElement.prototype.description = function(definitions) {
  ...
  if (this.$type) {
    typeName = util.splitNSName(this.$type).name;
    ns = definitions.xmlns[util.splitNSName(this.$type).namespace];
...
WSDL.prototype.xmlToObject = function(xml) {
  ...
  p.onopentag = function(node) {
    ...
    var name = util.splitNSName(nsName).name,
      attributeName,
  ...
  p.onclosetag = function(nsName) {
    var cur = stack.pop(),
      ...
      topSchema = top.schema,
      name = util.splitNSName(nsName).name;
  ...
  p.ontext = function(text) {
    ...
    var name = util.splitNSName(top.schema).name,
      value;
...
WSDL.prototype.objectToRpcXML = function(name, params, namespace, xmlns) {
  ...
  namespace = namespace || util.findKey(defs.xmlns, xmlns);
  ...
  for (var key in params) {
    if (key !== nsAttrName) {
      ...
      parts.push(['<', key, '>'].join(''));
      parts.push((typeof value === 'object') ? this.objectToXML(value) : util.xmlEscape(value));
      parts.push(['</', key, '>'].join(''));
    }
  }
  parts.push(['</', namespace, ':', name, '>'].join(''));
