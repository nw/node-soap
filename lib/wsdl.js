...
function Element(nsName, attrs) {
  var parts = utils.splitNSName(nsName);
...
Element.prototype.startElement = function(stack, nsName, attrs) {
  ...
  var ChildClass = this.allowedChildren[utils.splitNSName(nsName).name],
    element = null;
...
Element.prototype.endElement = function(stack, nsName) {
  if (this.nsName === nsName) {
    ...
    if (this !== stack[0]) {
      utils.extend(stack[0].xmlns, this.xmlns);
      // delete this.xmlns;
...
MessageElement.prototype.postProcess = function(definitions) {
  ...
  if (part.$element) {
    delete this.parts;
    nsName = utils.splitNSName(part.$element);
    ns = nsName.namespace;
    ...
    for (i = 0; part = this.children[i]; i++) {
      ...
      nsName = utils.splitNSName(part.$type);
...
OperationElement.prototype.postProcess = function(definitions, tag) {
  ...
  for (var i = 0, child; child = children[i]; i++) {
    ...
    var messageName = utils.splitNSName(child.$message).name;
...
BindingElement.prototype.postProcess = function(definitions) {
  var type = utils.splitNSName(this.$type).name,
...
ServiceElement.prototype.postProcess = function(definitions) {
  ...
  for (var i = 0, child; child = children[i]; i++) {
    if (child.name !== 'port')
      continue;
    var bindingName = utils.splitNSName(child.$binding).name;
...
ElementElement.prototype.description = function(definitions) {
  ...
  if (this.$type) {
    typeName = utils.splitNSName(this.$type).name;
    ns = definitions.xmlns[utils.splitNSName(this.$type).namespace];
...
var WSDL = function(definition, uri, options) {
  ...
  this.options = utils.extend(true, {}, options);
...
WSDL.prototype._processNextInclude = function(includes, callback) {
  ...
  var include_options = utils.extend({}, self.options);
  include_options.strict = false;

  fetch(includePath, include_options, function(err, wsdl) {
    if (err) {
      return callback(err);
    }
...
WSDL.prototype.xmlToObject = function(xml) {
  ...
  p.onopentag = function(node) {
    ...
    var name = utils.splitNSName(nsName).name,
      attributeName,
  ...
  p.onclosetag = function(nsName) {
    var cur = stack.pop(),
      ...
      topSchema = top.schema,
      name = utils.splitNSName(nsName).name;
  ...
  p.ontext = function(text) {
    ...
    var name = utils.splitNSName(top.schema).name,
      value;
...
function fetch(uri, options, callback){
  ...
  if (!/^http/.test(uri)) {
    ...
    utils.request(uri, null, options, function(err, response, definition) {
      if (err) {
        callback(err);
...
function load(url, options, callback){
  if(cache[url]) return callback(null, cache[url]); // cache
  
  if(typeof url !== 'string') return callback(new Error('no url'));
  
  fetch(url, options, function(err, wsdl){
    if(err) return callback(err);
    if(!wsdl) return callback(new Error('no wsdl returned'));
...
exports.cache = cache;
exports.fetch = fetch; // bypasses cache
exports.load = load;
exports.WSDL = WSDL;
